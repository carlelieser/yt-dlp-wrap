...
export default class YTDlpWrap {
	...
	constructor(binaryPath: string = executableName, shell: string = shellPath) {
	...
	private setDefaultOptions(options: YTDlpOptions): YTDlpOptions {
		if (!options.maxBuffer) options.maxBuffer = 1024 * 1024 * 1024;
		if (!options.shell) options.shell = this.shell;
		return options;
	}
